...
#     customers = list(range(1, n + 1))
#     random.shuffle(customers)
#     remaining_customers = n - (4 * (k-1) + 3)
#     for i in range(k):

#         solution[i].append(customers[4*i])
#         solution[i].append(customers[4*i + 1])
#         solution[i].append(customers[4*i + 2])
#         solution[i].append(customers[4*i + 3])
...
# def tabu_search(solution, max_iter=100000):
...
#                 delta = temp_length_max - temp_length_min - 1
#                 temp_mem_max, temp_mem_min = solution[max_idx], solution[min_idx]
#                 multi_transfer_point = solution[max_idx][:delta]

#                 solution[min_idx] = solution[max_idx][delta:]
#                 solution[max_idx] = solution[min_idx] + \
...
#         #         1,  int(k/3))][0], sorted_times[random.randint(
#         #             int(k/1.4), k-1)][0]
...
#             temp_length = len(solution[max_idx])
#             if temp_length >= 1:
#                 vertex = random.randint(
#                     int(temp_length / 1.4), int(temp_length / 1))
#                 temp_mem_max, temp_mem_min = solution[max_idx], solution[min_idx]
#                 multi_transfer_point = solution[max_idx][vertex:]

#                 solution[max_idx] = solution[max_idx][:vertex]
#                 solution[min_idx] = solution[min_idx] + multi_transfer_point
...
#                     # Thay vào đó, bạn cần hoàn nguyên lại toàn bộ solution
#                     solution[min_idx] = temp_mem_min
#                     solution[max_idx] = temp_mem_max
#                     continue
...
#             if temp_length >= 1:
#                 vertex = random.randint(
#                     int(temp_length / 3), int(temp_length / 1))
#                 multi_transfer_point = solution[max_idx][vertex:]
#                 temp_mem_max, temp_mem_min = solution[max_idx], solution[min_idx]
...
#     optimized_solution, optimized_times = tabu_search(initial_solution, 2000)
...
def tabu_search(solution, max_iter=200000):
    # Thay vì chỉ dùng deque, ta dùng thêm set để kiểm tra nhanh
    tabu_list = deque()
    tabu_set = set()
    ...
    for iteration in range(max_iter):
        ...
        if prop < 0.65:
        ...
        elif prop < 0.75 and k > 7:
            max_idx, min_idx = sorted_times[random.randint(
                0, int(k/3))][0], sorted_times[random.randint(int(k/4) + 1, k-1)][0]
            temp_length_max = len(solution[max_idx])
            temp_length_min = len(solution[min_idx])
            if temp_length_max >= 1:
            ...
            max_idx, min_idx = sorted_times[random.randint(
                0, int(k/2))][0], sorted_times[random.randint(int(k/2) + 1, k-1)][0]
            temp_length = len(solution[max_idx])
            if temp_length >= 1:
                vertex = random.randint(0, temp_length - 1)
                transfer_point = solution[max_idx][vertex]
